...
app.post('/getlicenseKey', async (req, res) => {
  ...
  try {
    const user = await FormData.findOne({ emailAddress: email, licenseKey: licenseKey });

    if (!user) {
      console.log("User not found or license key is incorrect");
      return res.status(400).json({ error: "User not found or license key is incorrect" });
    }

    console.log("User found and license key is correct");
    res.status(200).json({status:true, success: "User found and license key is correct" });
...
app.get('/download', (req, res) => {
  ...
  // Set headers for the response
  res.setHeader('Content-Disposition', `attachment; filename=DevClenerfile.zip`);
  res.setHeader('Content-Type', 'application/zip');
